...
Panier.afterRender = async () => {
    ...
    try {
        ...
        contenu.innerHTML = `
            <p class="panier-vide">
                Impossible de charger le panier (le serveur est-il lancé (npm run dev sur le port 3000) ?).
            </p>
        `;
    }
};
...
function ouvrirModaleCommande({ items, produits, total }) {
    ...
    overlay.querySelector('#btnConfirmerCommande').addEventListener('click', async (e) => {
        ...
        try {
            ...
            console.error(err);
            alert('Impossible de valider la commande (le serveur est-il lancé (npm run dev sur le port 3000) ?).');
            btn.disabled = false;
